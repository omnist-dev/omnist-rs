...
fn test_bigint_to_number_exact_precision() {
    ...
    let n1 = edges(vec![(
        "x",
        leaf(DocScalar::Int(num_bigint::BigInt::from(max_exact))),
    )]);
    assert!(materialize(&n1, Some(&schema)).is_ok());

    // -(2^53): exactly representable in f64
    let n1_neg = edges(vec![(
        "x",
        leaf(DocScalar::Int(num_bigint::BigInt::from(-max_exact))),
    )]);
    ...
    let n2 = edges(vec![(
        "x",
        leaf(DocScalar::Int(num_bigint::BigInt::from(inexact))),
    )]);
    let err = materialize(&n2, Some(&schema)).unwrap_err();
    assert!(
        err.errors()
            .iter()
            .any(|e| e.code == ErrorCode::TypeMismatch && e.path == "$.x")
    );

    // -(2^53 + 1): NOT exactly representable in f64
    let n2_neg = edges(vec![(
        "x",
        leaf(DocScalar::Int(num_bigint::BigInt::from(-inexact))),
    )]);
